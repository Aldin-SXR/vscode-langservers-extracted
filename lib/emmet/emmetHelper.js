...
function doComplete(monaco, model, position, syntax, emmetConfig) {
    ...
    if (isStyleSheetRes) {
        ...
        if (abbreviation.length > 4 && data_1.cssData.properties.some((x) => x.startsWith(abbreviation))) {
        ...
        if (expandedAbbr && expandedText.length) {
            ...
            completionItems = makeSnippetSuggestion(monaco, stylesheetCustomSnippetsKeys ?? [], abbreviation, abbreviation, abbreviationRange, expandOptions, 'Emmet Custom Snippet', false);
