...
function startServer(connection, runtime) {
    ...
    function withMarkdownIfUnsupported(capabilities) {
        ...
        if (!hover.contentFormat || hover.contentFormat.length === 0) {
            hover.contentFormat = [vscode_languageserver_1.MarkupKind.Markdown, vscode_languageserver_1.MarkupKind.PlainText];
        }
        ...
        if (!completionItem.documentationFormat || completionItem.documentationFormat.length === 0) {
            completionItem.documentationFormat = [vscode_languageserver_1.MarkupKind.Markdown, vscode_languageserver_1.MarkupKind.PlainText];
        }
        return result;
    }
    ...
    connection.onInitialize((params) => {
        ...
        languageModes = (0, languageModes_1.getLanguageModes)(initializationOptions?.embeddedLanguages || { css: true, javascript: true }, workspace, clientCapabilities, fileSystemProvider);
