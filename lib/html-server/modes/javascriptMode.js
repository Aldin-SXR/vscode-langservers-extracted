...
function getJavaScriptMode(documentRegions, languageId, workspace) {
    ...
    return {
        ...
        async doComplete(document, position, _documentContext) {
            ...
            const replaceRange = convertRange(jsDocument, (0, strings_1.getWordAtText)(documentText, offset, JS_WORD_REGEX));
            ...
            const items = completions.entries.map((entry, index) => {
                const data = {
                    languageId,
                    uri: document.uri,
                    offset: offset,
                ...
                const baseKind = entry.kind ? convertKind(entry.kind) : languageModes_1.CompletionItemKind.Property;
...
function isAfterPropertyAccess(text, offset) {
    let i = offset - 1;
    while (i >= 0) {
