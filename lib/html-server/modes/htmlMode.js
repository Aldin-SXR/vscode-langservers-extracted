...
function getHTMLMode(htmlLanguageService, workspace) {
    ...
    return {
        ...
        async doComplete(document, position, documentContext, settings = workspace.settings) {
            ...
            const emmetCompletions = allowEmmet ? (0, lspAdapter_1.getEmmetCompletions)(document, position, 'html', emmetConfig) : undefined;
            const htmlItems = completionList?.items || [];
            htmlItems.forEach(item => {
                if (!item.sortText && item.kind === languageModes_1.CompletionItemKind.Text) {
...
function isInsideStartTag(document, position, htmlLanguageService) {
    const offset = document.offsetAt(position);
    const scanner = htmlLanguageService.createScanner(document.getText());
    let token = scanner.scan();
    while (token !== undefined && token !== null && token !== languageModes_1.TokenType.EOS) {
        if (scanner.getTokenEnd() >= offset) {
            const state = scanner.getScannerState();
            return state === vscode_html_languageservice_1.ScannerState.AfterOpeningStartTag || state === vscode_html_languageservice_1.ScannerState.WithinTag || state === vscode_html_languageservice_1.ScannerState.AfterAttributeName || state === vscode_html_languageservice_1.ScannerState.BeforeAttributeValue;
        }
        token = scanner.scan();
    }
    return false;
